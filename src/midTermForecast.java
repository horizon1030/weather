...
public class midTermForecast {
    ...
    public MidTermWeathr getWeather_midTerm_2_date(LocalDateTime date)
    {
        type = "중기육상예보조회";
        //강수 확률과 날씨(ex 구름많음 최대 10일)
        int num = 12;
        ...
        MidTermWeathr midTermWeathr = new MidTermWeathr();
        // if (isAm)
        // {
        //     midTermWeathr.wfAm =  item.get("wf" + (n + 3) + "Am").toString();
        //     midTermWeathr.rnStAm = item.get("rnSt" + (n + 3) + "Am").toString();
        // }
        // else
        // {
        //     midTermWeathr.wfPm =  item.get("wf" + (n + 3) + "Pm").toString();
        //     midTermWeathr.rnStPm = item.get("rnSt" + (n + 3) + "Pm").toString();
        // }
        return midTermWeathr;
    }
